...
var BlogSection = function () {
    return (<motion.section className="py-8 px-4 md:px-12 relative z-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 }}>
      <div className="container mx-auto mb-12">
        <motion.h2 className="font-space font-bold text-3xl md:text-4xl mb-8 text-center neon-text-purple" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
          Latest Tech Insights
        </motion.h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {blogPosts.map(function (post, index) { return (<Card3D key={post.id} className={"rounded-xl overflow-hidden bg-space-blue p-4 md:p-6 neon-border-".concat(post.color)}>
              <div className="aspect-video bg-deep-space rounded-lg mb-4 overflow-hidden">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover"/>
              </div>
              <h3 className={"font-space font-bold text-xl mb-2 neon-text-".concat(post.color)}>{post.title}</h3>
              <p className="text-text-light opacity-80 text-sm mb-4">{post.description}</p>
              <motion.a href="#" className={"inline-block neon-button-".concat(post.color, " rounded-lg py-2 px-4 text-sm font-space")} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                Read More
              </motion.a>
            </Card3D>); })}
        </div>
